checkContrApptionInput <- function(data, annotation)
{
  ...
  checkTypes <- function()
  {
    ...
    inputs <- list(data = data, annotation = annotation)

    # enumerate the data args
    for(i in seq_along(inputs)) {
      # the string name of the argument
      inputName <- names(inputs)[i]
      # get the class of the data structure at the index of that argument
      inputClass <- class(inputs[[i]])[1] # get the first name returned by 'class()'
      # if that name isn't on the guest list...
      if(!(inputClass %in% c("data.frame", "matrix"))){
        # kick'em out
        stop(
          paste0(
